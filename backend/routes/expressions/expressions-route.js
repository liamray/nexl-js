...
router.get('/*', function (req, res) {
	...
	security.hasReadPermission(username).then(
		(hasPermission) => {
			if (!hasPermission) {
		...
		(err) => {
			logger.log.error('Failed to evaluate nexl expression for [%s] user. Reason : [%s]', username, err);
